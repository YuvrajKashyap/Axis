...
const statusTag: Record<DomainListStatus, { label: string; cls: string }> = {
  ...
  ARCHIVED: { label: "ARCHIVED", cls: "text-zinc-500 border-zinc-500/40" },
};

const sectionLabel: Record<DomainListStatus, string> = {
  ALIGNED: "IN ORDER",
  NEUTRAL: "WATCH",
  DRIFTING: "CRITICAL",
  ARCHIVED: "ARCHIVE",
...
export default async function Design4() {
  ...
  const lead: DomainListItem | undefined = domains[0];
  const secondary: DomainList = domains.slice(1);
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <header className="border-b-2 border-white px-8 md:px-12 py-5">
        ...
        <div className="flex items-center justify-between mt-3 text-[10px] font-mono tracking-widest uppercase text-zinc-700">
          ...
          <span>
            {domains.filter((domain: DomainListItem) => domain.status === "DRIFTING").length} critical
          </span>
        </div>
      </header>
      ...
      <div className="px-8 md:px-12 py-8 grid grid-cols-1 md:grid-cols-12 gap-0 md:divide-x md:divide-zinc-900">
        ...
        {lead && (
          <div className="md:col-span-7 md:pr-10 pb-8 md:pb-0">
            <div className="mb-6">
              <span className={`text-[10px] font-mono tracking-[0.4em] uppercase border px-2 py-0.5 ${statusTag[lead.status].cls}`}>
                {sectionLabel[lead.status]}
              </span>
            </div>
        ...
        <div className="md:col-span-5 md:pl-10 flex flex-col divide-y divide-zinc-900">
          {secondary.map((domain: DomainListItem) => {
            const status = domain.status;

            return (
              <div key={domain.id} className="py-8 first:pt-0">
                <div className="mb-4">
